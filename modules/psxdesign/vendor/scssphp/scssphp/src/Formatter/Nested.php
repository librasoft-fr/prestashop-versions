 ...
 * @internal
 */
class Nested extends \PrestaShop\Module\PsxDesign\Vendor\ScssPhp\ScssPhp\Formatter
{
    /**
     * @var int
     */
    ...
    protected function block(\PrestaShop\Module\PsxDesign\Vendor\ScssPhp\ScssPhp\Formatter\OutputBlock $block)
    {
        ...
        $isMediaOrDirective = \in_array($block->type, [\PrestaShop\Module\PsxDesign\Vendor\ScssPhp\ScssPhp\Type::T_DIRECTIVE, \PrestaShop\Module\PsxDesign\Vendor\ScssPhp\ScssPhp\Type::T_MEDIA]);
        $isSupport = $block->type === \PrestaShop\Module\PsxDesign\Vendor\ScssPhp\ScssPhp\Type::T_DIRECTIVE && $block->selectors && \strpos(\implode('', $block->selectors), '@supports') !== \false;
        while ($block->depth < \end($depths) || $block->depth == 1 && \end($depths) == 1) {
            \array_pop($depths);
            $this->depth--;
            if (!$this->depth && ($block->depth <= 1 || !$this->indentLevel && $block->type === \PrestaShop\Module\PsxDesign\Vendor\ScssPhp\ScssPhp\Type::T_COMMENT) && ($block->selectors && !$isMediaOrDirective || $previousHasSelector)) {
        ...
        $previousEmpty = $block->type === \PrestaShop\Module\PsxDesign\Vendor\ScssPhp\ScssPhp\Type::T_COMMENT;
        $previousHasSelector = \false;
