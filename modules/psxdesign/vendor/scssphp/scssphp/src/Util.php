...
class Util
{
     ...
     */
    public static function checkRange($name, \PrestaShop\Module\PsxDesign\Vendor\ScssPhp\ScssPhp\Base\Range $range, $value, $unit = '')
    {
        $val = $value[1];
        $grace = new \PrestaShop\Module\PsxDesign\Vendor\ScssPhp\ScssPhp\Base\Range(-1.0E-5, 1.0E-5);
        if (!\is_numeric($val)) {
            throw new \PrestaShop\Module\PsxDesign\Vendor\ScssPhp\ScssPhp\Exception\RangeException("{$name} {$val} is not a number.");
        }
        ...
        throw new \PrestaShop\Module\PsxDesign\Vendor\ScssPhp\ScssPhp\Exception\RangeException("{$name} {$val} must be between {$range->first} and {$range->last}{$unit}");
    }
    /**
